                    ...
                    <AnimatePresence>
                      {productsDropdownOpen && (
                        <motion.div
                          ...
                          className="absolute left-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-neutral-200 py-3 px-2 z-50 overflow-hidden"
                          onMouseLeave={() => setProductsDropdownOpen(false)}
                        >
                          <div className="px-3 py-1.5 border-b border-neutral-100 mb-2 flex items-center justify-between">
                            <span className="text-[11px] font-extrabold text-[#E86A2D] uppercase tracking-wider flex items-center gap-1.5">
                              <Package className="w-3.5 h-3.5" />
                              <span>6 Product Divisions</span>
                            </span>
                            <span className="text-[10px] text-neutral-400 font-medium">Select Page</span>
                          </div>
              ...
              <div className="grid gap-1">
                {navItems.map((item) => (
                  <div key={item.id} className="space-y-1">
                    ...
                    {/* Sub-list of 6 product pages when item is products */}
                    {item.id === 'products' && (
                      <div className="pl-3 pr-1 py-1 space-y-1 bg-neutral-50 rounded-xl border border-neutral-200/60 my-1">
                        <p className="text-[10px] font-extrabold uppercase tracking-wider text-[#E86A2D] px-3 pt-1">
                          Product Pages Range:
                        </p>
                        {PRODUCT_DIVISIONS.map((div) => (
                          <button
                            key={div.id}
                            onClick={() => handleCategoryClick(div.category)}
                            className="w-full text-left px-3 py-1.5 text-xs font-semibold text-neutral-700 hover:text-[#E86A2D] hover:bg-neutral-100 rounded-lg transition-colors flex items-center justify-between"
                          >
